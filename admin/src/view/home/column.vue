<template>
    <div class="c_right_box">
        <div class="c_right_top">
            <router-link to="index" class="on">栏目列表</router-link>
            <a href="javascript:;" @click="handleEdit()">添加栏目</a>
            <a href="javascript:;" @click="handleDelete()">删除栏目</a>
        </div>
        <el-table
            ref="multipleTable"
            :data="list"
        ...
        >
            ...
            <el-table-column label="栏目名称" prop="name" width="200"></el-table-column>
            <el-table-column label="栏目链接" prop="url"></el-table-column>
...
export default {
    ...
    methods : {
        // 加载数据
		appData () {
            ajax('home/column',{page:this.page} , rs => {
        ...
        // 更改排序
        handleRank (id,rank) {
            rankAjax('admin_column' , id , rank)
        },
        // 状态开关
        handleSwitch (id , val , field) {
            switchAjax('admin_column' , id , val , field)
        },
        // 添加&编辑
        handleEdit (row = ''){
            this.dialogFormVisible = true;
            this.form = row ? row : {};
            this.formMsg = (row ? '修改' : '添加')+'栏目';
        },
        // 提交
        submit () {
            ajax('home/columnSubmit', this.form , rs => {
                if(rs.ret == 1){
