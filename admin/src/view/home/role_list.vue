<template>
    <div class="c_right_box">
        <div class="c_right_top">
            <a href="javascript:;" class="on">角色列表</a>
            <a href="javascript:;" @click="handleEdit()">添加角色</a>
            <a href="javascript:;" @click="handleDelete()">删除角色</a>
        </div>
        <el-table
            ref="multipleTable"
            :data="list"
...
export default {
    ...
    methods : {
        // 加载数据
		appData () {
            ajax('home/role_list',{page:this.page} , rs => {
        ...
        // 更改排序
        handleRank (id,rank) {
            rankAjax('admin_role' , id , rank)
        },
        // 状态开关
        handleSwitch (id , val , field) {
            switchAjax('admin_role' , id , val , field)
        },
        // 添加&编辑
        handleEdit (row = ''){
            this.dialogFormVisible = true;
            this.form = row ? row : {};
            this.formMsg = (row ? '修改' : '添加')+'角色';
        },
        // 提交
        submit () {
            ajax('home/roleSubmit', this.form , rs => {
                if(rs.ret == 1){
