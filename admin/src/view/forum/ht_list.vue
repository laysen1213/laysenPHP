<template>
    <div class="c_right_box">
        <div class="c_right_top">
            <a href="javascript:;" class="on">话题列表</a>
            <a href="javascript:;" @click="handleEdit()">添加话题</a>
            <a href="javascript:;" @click="handleDelete()">删除话题</a>
        </div>
        <el-table
            ref="multipleTable"
            :data="list"
        ...
        >
            ...
            <el-table-column label="话题名称" prop="name" width="200"></el-table-column>
            <el-table-column label="封面">
                <template slot-scope="scope">
                    <img :src="scope.row.cover" alt="" height="80">
                </template>
            </el-table-column>
            <el-table-column label="帖子数" prop="tzs"></el-table-column>
            <el-table-column label="关注数" prop="gzs"></el-table-column>
        ...
        <el-dialog :title="formMsg" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                ...
                <el-form-item label="详细内容" :label-width="labelWidth">
                    <el-input v-model="form.content" type="textarea" style="max-width:400px;"></el-input>
                </el-form-item>
...
export default {
    ...
    methods : {
        // 加载数据
		appData () {
            ajax('forum/ht_list',{page:this.page} , rs => {
        ...
        // 更改排序
        handleRank (id,rank) {
            rankAjax('forum_ht' , id , rank)
        },
        // 状态开关
        handleSwitch (id , val , field) {
            switchAjax('forum_ht' , id , val , field)
        },
        // 添加&编辑
        handleEdit (row = ''){
            this.dialogFormVisible = true;
            this.form = row ? row : {};
            this.formMsg = (row ? '修改' : '添加')+'话题';
            this.cover = row ? row.cover : '';
        },
        // 提交
        submit () {
            this.form.cover = this.$refs.cover.imgSrc;
            ajax('forum/htSubmit', this.form , rs => {
                if(rs.ret == 1){
